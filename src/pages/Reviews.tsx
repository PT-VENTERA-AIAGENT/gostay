    ...
    <PageTransition>
      <div className="p-4 md:p-6 space-y-4 md:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-foreground">Reviews</h1>
        ...
        {isLoading ? (
          ...
          <div className="p-6 text-center text-sm text-destructive">Gagal memuat ulasan.</div>
        ) : reviews.length === 0 ? (
          <div className="bg-card rounded-xl border border-border p-10 text-center">
            <Star className="w-10 h-10 text-muted-foreground/40 mx-auto mb-3" />
            <p className="text-sm font-medium text-foreground mb-1">Belum ada ulasan</p>
            <p className="text-xs text-muted-foreground">Ulasan tamu akan muncul di sini setelah mereka menilai menginap.</p>
          </div>
        ) : (
            ...
            {reviews.map((r) => (
              <motion.div key={r.id} variants={staggerItem} className={`flex flex-col h-full bg-card rounded-xl border p-4 md:p-5 ${r.is_published ? "border-border" : "border-dashed border-muted-foreground/40 opacity-70"}`}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  {!r.is_published && <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">Disembunyikan</span>}
                </div>
