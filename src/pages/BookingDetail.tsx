...
export default function BookingDetail() {
  ...
  if (error || !booking) {
    return (
      <PageTransition>
        <div className="p-6 text-center text-sm text-destructive">{t("Booking not found.")}</div>
      </PageTransition>
    );
  }
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="lg:col-span-2 space-y-4 md:space-y-6">
            <motion.div variants={staggerItem} className="bg-card rounded-xl border border-border p-4 md:p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 text-sm">
                ...
                <div><span className="text-muted-foreground">{t("Phone")}</span><p className="font-medium text-foreground mt-0.5">{booking.customers?.phone ?? "—"}</p></div>
                <div><span className="text-muted-foreground">{t("Nationality")}</span><p className="font-medium text-foreground mt-0.5">{booking.customers?.nationality ?? "—"}</p></div>
              </div>
            </motion.div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-4 md:space-y-6">
            <motion.div variants={staggerItem} className="bg-card rounded-xl border border-border p-4 md:p-5">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">{t("Paid")}</span><span className="font-medium text-foreground">{formatIDR(booking.amount_paid)}</span></div>
              </div>
            </motion.div>
            ...
            <motion.div variants={staggerItem} className="bg-card rounded-xl border border-border p-4 md:p-5">
              <h2 className="font-semibold text-foreground mb-4">{t("Nightly Breakdown")}</h2>
