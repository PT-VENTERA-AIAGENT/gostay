...
import { ArrowLeft, CheckCircle, LogIn, LogOut, XCircle, AlertTriangle, Clock, FileText, User, MapPin, CreditCard } from "lucide-react";
...
const booking = {
  ...
  nightlyBreakdown: [
    { date: "2026-04-01", price: 1250000 },
    { date: "2026-04-02", price: 1250000 },
    { date: "2026-04-03", price: 1250000 },
  ],
  auditLog: [
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-foreground">{booking.reference}</h1>
        ...
        <div className="flex items-center gap-2">
          {booking.status === "pending" && (
            <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-info text-info-foreground text-sm font-semibold hover:opacity-90 transition-opacity">
              <CheckCircle className="w-4 h-4" /> Confirm
            </button>
          )}
          {booking.status === "confirmed" && (
            <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity">
          ...
          {["pending", "confirmed"].includes(booking.status) && (
            <>
              <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-destructive text-destructive text-sm font-medium hover:bg-destructive/10 transition-colors">
                <XCircle className="w-4 h-4" /> Cancel
              </button>
              <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-warning text-warning text-sm font-medium hover:bg-warning/10 transition-colors">
        ...
        <div className="col-span-2 space-y-6">
          ...
          <div className="bg-card rounded-xl border border-border p-5">
            ...
            <div className="space-y-3 text-sm">
              <div>
                <span className="text-muted-foreground text-xs uppercase tracking-wide">Special Requests</span>
                <p className="text-foreground mt-1">{booking.specialRequests}</p>
              </div>
              <div>
                <span className="text-muted-foreground text-xs uppercase tracking-wide">Internal Notes</span>
                <p className="text-foreground mt-1">{booking.internalNotes}</p>
              </div>
            </div>
          </div>
