  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/rooms/types" className="w-9 h-9 rounded-lg border border-border flex items-center justify-center text-muted-foreground hover:bg-muted transition-colors">
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-foreground">{mockType.name}</h1>
        ...
        <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-destructive bg-card text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors">
            ...
            <div className="space-y-3">
              {mockType.priceOverrides.map((o, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  ...
                  <span className="text-sm font-semibold text-primary">{formatIDR(o.pricePerNight)}/night</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-card rounded-xl border border-border p-5">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Base Price</span><span className="font-semibold text-foreground">{formatIDR(mockType.basePrice)}/night</span></div>
          ...
          <div className="bg-card rounded-xl border border-border p-5">
            ...
            <div className="space-y-2">
              {mockType.rooms.map((r) => (
                <div key={r.number} className="flex items-center justify-between p-2.5 bg-muted rounded-lg">
                  <div>
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                    ...
                    r.status === "occupied" ? "bg-info/10 text-info" :
